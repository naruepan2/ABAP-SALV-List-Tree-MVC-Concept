...
*----------------------------------------------------------------------*
CLASS lcl_controller DEFINITION INHERITING FROM zcl_mvcfw_base_controller.
  PUBLIC SECTION.
...
CLASS lcl_controller IMPLEMENTATION.
            ...
            IF row GT 0.
              TRY.
                  me->create_new_view_instance( iv_stack_name = 'SUB01'
                                                ir_event_param = NEW #( row_double_click    = row
  ...
  METHOD populate_setup_before_display.
    IF cr_salv_param->stack_name EQ 'SUB01'.
      cr_salv_param->start_column = 1.
      cr_salv_param->start_line   = 1.
      cr_salv_param->end_column   = 100.
      cr_salv_param->end_line     = 20.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
