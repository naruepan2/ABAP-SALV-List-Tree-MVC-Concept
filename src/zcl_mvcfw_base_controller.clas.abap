...
public section.
  ...
  constants MC_DISPLAY_SALV_LIST type CHAR01 value '1' ##NO_TEXT.
  constants MC_DISPLAY_SALV_TREE type CHAR01 value '2' ##NO_TEXT.
  data MO_SSCR type ref to ZCL_MVCFW_BASE_SSCR .
  data MO_MODEL type ref to ZCL_MVCFW_BASE_MODEL .
  data MO_LIST_VIEW type ref to ZCL_MVCFW_BASE_SALV_LIST_VIEW .
  data MO_TREE_VIEW type ref to ZCL_MVCFW_BASE_SALV_TREE_VIEW .
  ...
  methods SET_STACK_NAME
    ...
    exporting
      ...
      !EO_CONTROLLER type ref to ZCL_MVCFW_BASE_CONTROLLER .
  ...
  methods CREATE_NEW_VIEW_INSTANCE
    importing
      ...
      !IO_SALV_VIEW type ref to ZCL_MVCFW_BASE_SALV_LIST_VIEW optional
      !IO_TREE_VIEW type ref to ZCL_MVCFW_BASE_SALV_TREE_VIEW optional
      !IR_EVENT_PARAM type ref to ZCL_MVCFW_BASE_MODEL=>TY_EVT_PARAM
  ...
  methods HANDLE_LIST_F4_REQUEST
    importing
      ...
      !SALV_VIEW type ref to ZCL_MVCFW_BASE_SALV_LIST_VIEW optional
    changing
...
private section.
  ...
  methods _VALIDATE_DISPLAY_TYPE
    importing
      !IV_DISPLAY_TYPE type CHAR1
    returning
      value(RV_DISPLAY_TYPE) type CHAR1 .
  ...
  methods _SET_MODEL
    ...
    exporting
      !EO_MODEL type ref to ZCL_MVCFW_BASE_MODEL
...
CLASS ZCL_MVCFW_BASE_CONTROLLER IMPLEMENTATION.
  ...
  METHOD create_new_view_instance.
    ...
    TRY.
        lo_model ?= COND #( WHEN io_model IS BOUND THEN io_model
                            ...
                            ELSE mo_model ).
      CATCH cx_sy_move_cast_error.
        lo_model ?= COND #( WHEN lmo_current_model IS BOUND THEN lmo_current_model
                            ELSE mo_model ).
    ENDTRY.
    TRY.
    ...
    ENDTRY.
    TRY.
        lo_tree_view ?= COND #( WHEN io_tree_view IS BOUND THEN io_tree_view
                                WHEN lmo_current_tree_view IS BOUND THEN lmo_current_tree_view
                                ELSE mo_tree_view ).
      CATCH cx_sy_move_cast_error.
        lo_tree_view ?= COND #( WHEN lmo_current_tree_view IS BOUND THEN lmo_current_tree_view
                                ELSE mo_tree_view ).
    ENDTRY.

    _get_abap_call_stack( )->_set_events_param_to_model( io_model       = lo_model
                                                         ir_event_param = ir_event_param ).
  ...
  METHOD display.
    ...
    DATA lv_display_type TYPE char01.
...
*      ENDIF.
  ...
  METHOD handle_list_link_click.
    ...
    salv_view->get_view_instance( )->refresh( ).
  ENDMETHOD.
...
*      ENDIF.
  ...
  METHOD _check_setup_before_display.
    ...
    ELSEIF cr_tree_param IS SUPPLIED.
      ...
      IF lmr_tree_param->stack_name NE lmv_current_stack.
        lmr_tree_param->stack_name = lmv_current_stack.
      ENDIF.
    ENDIF.
  ENDMETHOD.
    ...
    IF io_model IS BOUND.
      CASE lmv_triggered_evt.
        WHEN 'HANDLE_SALV_LINK_CLICK'.
          io_model->set_link_click_paramter( row    = ir_event_param->row_link_click
                                             column = ir_event_param->column_link_click ).
        WHEN 'HANDLE_SALV_DOUBLE_CLICK'.
          io_model->set_double_click_paramter( row    = ir_event_param->row_double_click
                                               column = ir_event_param->column_double_click ).
        WHEN 'HANDLE_SALV_ADD_FUNCTION'
          OR 'HANDLE_SALV_AFTER_FUNCTION'
          OR 'HANDLE_SALV_BEFORE_FUNCTION'.
          io_model->set_function_paramter( salv_function = ir_event_param->salv_function ).
        WHEN 'HANDLE_SALV_END_OF_PAGE'.
          io_model->set_top_end_of_page_paramter( r_end_of_page = ir_event_param->r_end_of_page
                                                  page          = ir_event_param->page ).
        WHEN 'HANDLE_SALV_TOP_OF_PAGE'.
          io_model->set_top_end_of_page_paramter( r_top_of_page = ir_event_param->r_top_of_page
                                                  page          = ir_event_param->page
                                                  table_index   = ir_event_param->table_index ).
        WHEN 'HANDLE_SALV_CHECK_CHANGED_DATA'.
          io_model->set_check_changed_data_param( t_modified_cells      = ir_event_param->t_modified_cells
                                                  t_good_cells          = ir_event_param->t_good_cells
                                                  t_deleted_rows        = ir_event_param->t_deleted_rows
                                                  t_inserted_rows       = ir_event_param->t_inserted_rows
                                                  rt_modified_data_rows = ir_event_param->rt_modified_data_rows
                                                  o_ui_data_modify      = ir_event_param->o_ui_data_modify
                                                  o_ui_edit_protocol    = ir_event_param->o_ui_edit_protocol ).
      ENDCASE.
    ENDIF.
  ENDMETHOD.
