...
public section.
  ...
  data MT_SELOPT_T type RSPARAMS_TT read-only .
  data MT_SELOPT_T_LONG type FMRPF_RSPARAMSL_255_T read-only .
  ...
  methods SET_FUNCTION_PARAMTER
    importing
      !DISPLAY_TYPE type SALV_DE_CONSTANT default ZCL_MVCFW_BASE_CONTROLLER=>MC_DISPLAY_SALV_LIST
...
* if requested -> propagate error with given type to caller
  ...
  METHOD set_function_paramter.
    CASE display_type.
      WHEN zcl_mvcfw_base_controller=>mc_display_salv_list.
        IF salv_function IS SUPPLIED.
          mv_evt_params-evt_list-salv_function = salv_function.
        ENDIF.
      WHEN zcl_mvcfw_base_controller=>mc_display_salv_tree.
        IF salv_function IS SUPPLIED.
