...
public section.
  ...
  types:
    BEGIN OF ts_fname,
      ...
      END OF ts_fname .
  types:
    tt_fname TYPE SORTED TABLE OF ts_fname WITH UNIQUE KEY ref_field ref_table .
  types:
    BEGIN OF ts_incl_outtab_ext,
        ...
        alv_s_color  TYPE lvc_emphsz,             "Simple row color coding
  ...
  data T_EDITABLE_COLS type TT_FNAME read-only .
  data T_CELL_TYPE type SALV_T_INT4_COLUMN read-only .
  data T_CHECKBOX_TYPE type TT_FNAME read-only .
  data S_EVT_PARAMS type TS_EVT_PARAMS read-only .
  ...
  methods SET_EDITABLE_CELL
    importing
      !IV_FNAME type LVC_RFNAME
      !IV_TNAME type LVC_RTNAME optional
      !IV_DISABLED type ABAP_BOOL default SPACE
    changing
      !CT_STYLE type LVC_T_STYL
    returning
      value(RO_MODEL) type ref to ZCL_MVCFW_BASE_SALV_MODEL .
  methods SET_EDITABLE_ALL_CELLS
    importing
      !IS_DATA type ANY
      !IV_TNAME type LVC_RTNAME optional
      !IV_DISABLED type ABAP_BOOL default SPACE
  ...
  methods SET_FUNCTION_PARAMTER
    importing
      !DISPLAY_TYPE type SALV_DE_CONSTANT default ZCL_MVCFW_BASE_SALV_CONTROLLER=>C_DISPLAY_SALV_LIST
...
**    ENDLOOP.
  ...
  METHOD set_check_changed_data_list.
    IF t_modified_cells IS SUPPLIED.
      s_evt_params-evt_list-t_modified_cells = t_modified_cells.
    ENDIF.
    IF t_good_cells IS SUPPLIED.
      s_evt_params-evt_list-t_good_cells = t_good_cells.
    ENDIF.
    IF t_deleted_rows IS SUPPLIED.
      s_evt_params-evt_list-t_deleted_rows = t_deleted_rows.
    ENDIF.
    IF t_inserted_rows IS SUPPLIED.
      s_evt_params-evt_list-t_inserted_rows = t_inserted_rows.
    ENDIF.
    IF rt_modified_data_rows IS SUPPLIED.
      s_evt_params-evt_list-rt_modified_data_rows = rt_modified_data_rows.
    ENDIF.
    IF o_ui_data_modify IS SUPPLIED.
      s_evt_params-evt_list-o_ui_data_modify = o_ui_data_modify.
    ENDIF.
    IF o_ui_edit_protocol IS SUPPLIED.
      s_evt_params-evt_list-o_ui_edit_protocol = o_ui_edit_protocol.
    ENDIF.
  ENDMETHOD.
...
*    INCLUDE <cl_alv_control>.
    ...
    ELSE.
      ...
      IF NOT line_exists( t_editable_cols[ ref_field = ls_style-fieldname
                                           ref_table = |{ iv_tname CASE = UPPER }| ] ).
        INSERT VALUE #( ref_field = ls_style-fieldname
                        ref_table = |{ iv_tname CASE = UPPER }| )
                INTO TABLE t_editable_cols.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD set_function_paramter.
    CASE display_type.
      WHEN zcl_mvcfw_base_salv_controller=>c_display_salv_list.
        IF salv_function IS SUPPLIED.
          s_evt_params-evt_list-salv_function = salv_function.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_display_salv_tree.
        IF salv_function IS SUPPLIED.
          s_evt_params-evt_tree-salv_function = salv_function.
        ENDIF.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD set_link_double_click_to_list.
    CASE event_type.
      WHEN zcl_mvcfw_base_salv_controller=>c_list_link_click.
        IF row IS SUPPLIED.
          s_evt_params-evt_list-row_link_click = row.
        ENDIF.
        IF column IS SUPPLIED.
          s_evt_params-evt_list-column_link_click = column.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_list_double_click.
        IF row IS SUPPLIED.
          s_evt_params-evt_list-row_double_click = row.
        ENDIF.
        IF column IS SUPPLIED.
          s_evt_params-evt_list-column_double_click = column.
        ENDIF.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD set_link_double_key_check_expd.
    CASE event_type.
      WHEN zcl_mvcfw_base_salv_controller=>c_tree_link_click.
        IF node_key IS SUPPLIED.
          s_evt_params-evt_tree-node_key_link_click = node_key.
        ENDIF.
        IF columnname IS SUPPLIED.
          s_evt_params-evt_tree-columnname_link_click = columnname.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_tree_double_click.
        IF node_key IS SUPPLIED.
          s_evt_params-evt_tree-node_key_double_click = node_key.
        ENDIF.
        IF columnname IS SUPPLIED.
          s_evt_params-evt_tree-columnname_double_click = columnname.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_tree_keypress.
        IF node_key IS SUPPLIED.
          s_evt_params-evt_tree-node_key_keypress = node_key.
        ENDIF.
        IF columnname IS SUPPLIED.
          s_evt_params-evt_tree-columnname_keypress = columnname.
        ENDIF.
        IF key IS SUPPLIED.
          s_evt_params-evt_tree-keypress = key.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_tree_checkbox_change.
        IF node_key IS SUPPLIED.
          s_evt_params-evt_tree-node_key_checkbox_change = node_key.
        ENDIF.
        IF columnname IS SUPPLIED.
          s_evt_params-evt_tree-columnname_chkbox_change = columnname.
        ENDIF.
        IF key IS SUPPLIED.
          s_evt_params-evt_tree-checked = checked.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_tree_expand_empty_foldr.
        IF node_key IS SUPPLIED.
          s_evt_params-evt_tree-node_key_exp_empty_foldr = node_key.
        ENDIF.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD set_salv_checkbox.
    ...
    ELSE.
      ...
      IF NOT line_exists( t_checkbox_type[ ref_field = ls_style-columnname
                                           ref_table = |{ iv_tname CASE = UPPER }| ] ).
        INSERT VALUE #( ref_field = ls_style-columnname
                        ref_table = |{ iv_tname CASE = UPPER }| )
                INTO TABLE t_checkbox_type.
      ENDIF.
    ENDIF.
  ENDMETHOD.
...
*--------------------------------------------------------------------*
    ...
    CASE display_type.
      WHEN zcl_mvcfw_base_salv_controller=>c_display_salv_list.
        IF r_top_of_page IS SUPPLIED.
          s_evt_params-evt_list-r_top_of_page = r_top_of_page.
        ENDIF.
        IF r_end_of_page IS SUPPLIED.
          s_evt_params-evt_list-r_end_of_page = r_end_of_page.
        ENDIF.
        IF page IS SUPPLIED.
          s_evt_params-evt_list-page = page.
        ENDIF.
        IF table_index IS SUPPLIED.
          s_evt_params-evt_list-table_index = table_index.
        ENDIF.
      WHEN zcl_mvcfw_base_salv_controller=>c_display_salv_tree.
        IF r_top_of_page IS SUPPLIED.
          s_evt_params-evt_tree-r_top_of_page = r_top_of_page.
        ENDIF.
        IF r_end_of_page IS SUPPLIED.
          s_evt_params-evt_tree-r_end_of_page = r_end_of_page.
        ENDIF.
        IF page IS SUPPLIED.
          s_evt_params-evt_tree-page = page.
        ENDIF.
        IF table_index IS SUPPLIED.
          s_evt_params-evt_tree-table_index = table_index.
        ENDIF.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD clear_evt_params.
    ...
    CASE abap_true.
      WHEN iv_evt_list.
        CLEAR s_evt_params-evt_list.
      WHEN iv_evt_tree.
        CLEAR s_evt_params-evt_tree.
      WHEN iv_evt_all.
        CLEAR s_evt_params.
    ENDCASE.
  ENDMETHOD.
...
*    INCLUDE <cl_alv_control>.
    ...
    LOOP AT CAST cl_abap_structdescr( cl_abap_typedescr=>describe_by_data( is_data ) )->components INTO DATA(ls_comp)
      WHERE type_kind <> cl_abap_typedescr=>typekind_struct1
        AND type_kind <> cl_abap_typedescr=>typekind_struct2
        AND type_kind <> cl_abap_typedescr=>typekind_table
        AND type_kind <> cl_abap_typedescr=>typekind_xsequence
        AND type_kind <> cl_abap_typedescr=>typekind_xstring
        AND type_kind <> cl_abap_typedescr=>typekind_hex
        AND type_kind <> cl_abap_typedescr=>typekind_dref
        AND type_kind <> cl_abap_typedescr=>typekind_class.
      READ TABLE ct_style ASSIGNING FIELD-SYMBOL(<lfs_style>)
        WITH TABLE KEY fieldname = ls_comp-name.
      IF sy-subrc EQ 0.
        <lfs_style>-style  = l_style.
      ELSE.
        CLEAR ls_style.
